...
public class Mannequin extends CustomMobOption {
    ...
    /**
     * Represents the mannequin's elytra's texture.
     */
    private static final String ELYTRA = "mob.mannequin-elytra";
    ...
    public List<ItemStack> getOptionItems() {
        ...
        items.add(getOptionItemStack(getElytraItem(), true, false));
    ...
    public ItemStack modifyOption(Player clicker, CustomMob customMob, String option, ClickType clickType) {
        switch (option.toLowerCase()) {
            ...
            case "elytra": {
                if (clickType.isRightClick()) {
                    elytra = "";
                    setOption(ELYTRA, elytra);
                } else {
                    this.editing = "Elytra";
                    sendChangeTextureMessage(clicker, "elytra's texture");
                }
                break;
            }
            ...
            case "pose": {
                ...
                setOption(POSE, pose != null ? getVariantName(pigVariant) : null);
    ...
    public CustomMobsItem getElytraItem() {
        CustomMobsItem item = new CustomMobsItem(Material.ELYTRA);
        item.setName("&b&lElytra texture");
        item.addLore("&eElytra's texture: &f" + (!elytra.isEmpty()? elytra : "None"));
        item.setOptionPersistentDataContainer(this.getClass().getSimpleName(), "Elytra");
        return item;
    }
    ...
    public CustomMobsItem getPoseItem() {
        ...
        String pose = pose == null ? "&fDefault" : "&f" + Utils.getSentenceCase(getVariantName(pigVariant));
        item.addLore("&eMannequin's pose: &f" + variant);
    ...
    public CustomMobsItem getDescriptionItem() {
        ...
        item.addLore("&eDescription: &f" + (!elytra.isEmpty()? elytra : "None"));
    ...
    public void sendChangeDescriptionMessage(Player player) {
        ...
        player.sendMessage(Utils.applyFormat(" &6&l- &fThe description supports &4c&co&6l&eo&ar&bs&f ( &* (&ebasic&f) and &#* (&#cbff0fhex&f) )"));
        player.sendMessage(Utils.applyFormat(" &6&l- &fType &ccancel &fin the chat to cancel."));
    }
